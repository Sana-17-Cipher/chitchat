...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void _showNewMessageToast(Map<String, dynamic> row) {
    final senderId = row['sender_id'] as String;
    final match = _conversations.firstWhere((c) => c['other_user_id'] == senderId, orElse: () => {'username': 'Someone'});
    final senderName = match['username'] ?? 'Someone';
    final preview = (row['content'] as String?)?.isNotEmpty == true ? row['content'] : (row['image_url'] != null ? '📷 Photo' : 'New message');

    scaffoldMessengerKey.currentState?.showSnackBar(
      SnackBar(
        content: Text('$senderName: $preview'),
        action: SnackBarAction(
          label: 'Open',
          onPressed: () {
            navigatorKey.currentState?.push(
              MaterialPageRoute(builder: (_) => ChatScreen(receiverId: senderId, receiverUsername: senderName)),
            );
          },
        ),
  ...
  Future<void> _signOut() async {
    ...
    if (userId != null) {
      await supabase.from('profiles').update({'online': false, 'last_seen': DateTime.now().toIso8601String()}).eq('id', userId);
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            icon: CircleAvatar(radius: 16, backgroundImage: _myAvatarUrl != null ? NetworkImage(_myAvatarUrl!) : null, child: _myAvatarUrl == null ? const Icon(Icons.person, size: 18) : null),
            tooltip: 'Profile',
            onPressed: () {
              Navigator.push(context, MaterialPageRoute(builder: (_) => const ProfileScreen())).then((_) => _loadMyAvatar());
            },
          ),
          PopupMenuButton<String>(
            ...
            itemBuilder: (context) => [
              const PopupMenuItem(value: 'new_group', child: ListTile(leading: Icon(Icons.group_add_outlined), title: Text('New group'))),
              const PopupMenuItem(value: 'sign_out', child: ListTile(leading: Icon(Icons.logout), title: Text('Sign out'))),
            ],
          ),
        ],
      ),
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          Navigator.push(context, MaterialPageRoute(builder: (_) => const NewChatScreen())).then((_) => _loadConversations());
        },
      ...
      body: CustomScrollView(
        slivers: [
          if (_groups.isNotEmpty) ...[
            const SliverToBoxAdapter(child: Padding(padding: EdgeInsets.fromLTRB(16, 12, 16, 4), child: Text('Groups', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.black54)))),
            SliverList(
              delegate: SliverChildBuilderDelegate(
                (context, index) {
                  final group = _groups[index];
                  return ListTile(
                    leading: CircleAvatar(backgroundImage: group['avatar_url'] != null ? NetworkImage(group['avatar_url']) : null, child: group['avatar_url'] == null ? const Icon(Icons.group) : null),
          ...
          const SliverToBoxAdapter(child: Padding(padding: EdgeInsets.fromLTRB(16, 16, 16, 4), child: Text('Chats', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.black54)))),
          if (_loadingConversations)
            const SliverToBoxAdapter(child: Padding(padding: EdgeInsets.all(24), child: Center(child: CircularProgressIndicator())))
          else if (_conversations.isEmpty)
            const SliverToBoxAdapter(child: Padding(padding: EdgeInsets.all(24), child: Text('No conversations yet. Tap the chat button below to start one!', textAlign: TextAlign.center)))
          else
            SliverList(
              delegate: SliverChildBuilderDelegate(
                (context, index) {
                  ...
                  return Dismissible(
                    ...
                    child: ListTile(
                      leading: Stack(
                        children: [
                          CircleAvatar(backgroundImage: avatarUrl != null ? NetworkImage(avatarUrl) : null, child: avatarUrl == null ? Text((conv['username'] as String).isNotEmpty ? conv['username'][0].toUpperCase() : '?') : null),
                          if (isOnline)
                      ...
                      title: Text(conv['username'] ?? 'Unknown'),
                      ...
                      trailing: Column(
                        ...
                        children: [
                          ...
                          if (unread > 0) CircleAvatar(radius: 11, backgroundColor: Colors.red, child: Text('$unread', style: const TextStyle(color: Colors.white, fontSize: 11))),
                        ],
                      ),
                      onTap: () {
                        Navigator.push(context, MaterialPageRoute(builder: (_) => ChatScreen(receiverId: otherUserId, receiverUsername: conv['username']))).then((_) => _loadConversations());
                      },
                    ),
                  );
                },
